...
proc get_gitid_name {} {
    switch -glob -- [exec git describe --always --dirty] {
        *-dirty      {return [exec git rev-parse --short=8 HEAD]-dirty}
        default      {return [exec git rev-parse --short=8 HEAD]}
    }
}
...
create_project marble_zest_top $outputDir -part $part -force

# Provision to source additional TCL scripts
# Currently used for gtx_config.tcl and swap_gitid.tcl
foreach aux_tcl [lrange $argv 4 end] {
...
set gitid_for_filename [get_gitid_name]
...
set dest_dir "$outputDir/marble_zest_top.runs"
...
write_bitstream -force -bin_file marble_zest_top.$gitid_for_filename.bit
write_bitstream -force marble_zest_top.$gitid_for_filename.x.bit
