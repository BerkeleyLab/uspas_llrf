...
async def task_ddc_check(dut, f_config='USPAS'):
    ...
    clock = Clock(dut.clk, llrf.DSP_CLK_CYCLE, units="ns")
    ...
    dut.rx_phase_offset.value = int(llrf.rx.phase_off_deg / 360 * 2**19)
    ...
    for n, (nco, sig) in enumerate(zip(
            llrf.gen_sinusoidal(),
            llrf.gen_sinusoidal(amp_exp, phs_exp))):
        await RisingEdge(dut.clk)
        nco *= llrf.CORDIC_GAIN
...
async def test_rx_alsu(dut):
    await task_ddc_check(dut, f_config='ALSU')


@cocotb.test()
async def test_rx_uspas(dut):
    await task_ddc_check(dut, f_config='USPAS')


@cocotb.test()
async def test_rx_lemp(dut):
    await task_ddc_check(dut, f_config='LEMP')
