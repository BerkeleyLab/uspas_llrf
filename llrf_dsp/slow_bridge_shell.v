...
// {buf_stat1, buf_stat2, buf_count, 8'h0, tag, 8'h0, tag_old, adc_min_max, evr_timestamp, timestamp}
module slow_bridge_shell #(
    ...
    wire [N_CH*DW-1:0] adc_min;
    wire [N_CH*DW-1:0] adc_max;
    // assign adc_min = {adc_min[0], adc_min[1], ..., adc_min[7]};
    // assign adc_max = {adc_max[0], adc_max[1], ..., adc_max[7]};
    wire [SR_LEN2-1:0] adc_min_max = {adc_min, adc_max};
    genvar ix;
    generate for (ix=0; ix<N_CH; ix=ix+1) begin: gen_minmax
        minmax #(.width(DW)) mm_i (
            ...
            .reset  (slow_snap),
            .xmin   (adc_min[DW*(N_CH-ix-1) +: DW]),
            .xmax   (adc_max[DW*(N_CH-ix-1) +: DW])
        );
        end
    endgenerate
    ...
    always @(posedge dsp_clk) begin
        if (slow_op) begin
            ...
            slow_sr2 <= slow_snap ?  adc_min_max : {slow_sr2[SR_LEN2-DW-1:0], evr_ts_word};
