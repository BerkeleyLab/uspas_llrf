...
bool init_evr_gtx(void) {
    ...
    // EVR GTX FSM should be ready within 100 resets
    for (size_t ix=0; ix<100; ix++) {
        DELAY_MS(1);
        if (CHECK_BIT(read_lb_reg(GTX_RX_ALIGNED), 0))
            break;
    }
    printf("  %s: GTX_RESETS:  %d\n", __func__, read_lb_reg(GTX_RX_RESET_CNT));

    p = CHECK_BIT(read_lb_reg(GTX_RX_FSM_RESETDONE), 0); pass &= p;
    printf("  %s: GTX_RX_FSM_RESETDONE: %s\n", __func__, p ? "OK": "FAIL");
